			...
			<?php while ( have_posts() ) { ?>
				...
				<article id="post-<?php the_ID(); ?>" class="<?php hybrid_entry_class(); ?>">
					...
					<div class="entry-content">
						<?php the_content(); ?>
						<?php wp_link_pages( array( 'before' => '<p class="page-links">' . __( 'Pages:', 'kalervo' ), 'after' => '</p>' ) ); ?>
					</div><!-- .entry-content -->
				...
				<div class="attachment-meta">
					...
					<?php $gallery = gallery_shortcode( array( 'columns' => 6, 'numberposts' => -1, 'id' => $post->post_parent, 'exclude' => get_the_ID() ) ); ?>

					<?php if ( !empty( $gallery ) ) { ?>
						<div class="image-gallery">
							<h3><?php _e( 'Gallery', 'kalervo' ); ?></h3>
							<?php echo $gallery; ?>
						</div>
					<?php } ?>
